    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
              ...
              transcript_box = gr.Textbox(label="Transcription", max_lines=10,show_copy_button=True)
              llm_translate=gr.Textbox(label="LLM Translation Prompt",label="Copy & Paste this prompt in https://aistudio.google.com/", max_lines=20,show_copy_button=True)

        generate_btn.click(
            fn=process_media,
            inputs=[upload_media,number_of_speakers, input_lang, output_lang,method,task],
            outputs=[json_file, transcript_box,llm_translate]
        )
...
def main(debug=True, share=True): 
  ...
  custom_css = """.gradio-container { font-family: 'SF Pro Display', -apple-system, BlinkMacSystemFont, sans-serif; }"""
  demo = gr.TabbedInterface([demo1, demo2], ["Chatterbox TTS","Extract Video / Audio Text with speaker ID"], title="",theme=gr.themes.Soft(),css=custom_css)
  demo.queue().launch(share=share,debug=debug)
if __name__ == "__main__":
    main()       
