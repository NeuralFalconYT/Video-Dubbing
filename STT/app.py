...
def process_media(media_file,num_speakers, input_lang, output_lang):
  try:
    ...
    return json_transcription,readable_json
  except Exception as e:
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            ...
            with gr.Column():
              ...
              ranscript_box = gr.Textbox(label="Transcription For Dubbing", lines=4,show_copy_button=True)

        generate_btn.click(
            fn=process_media,
            inputs=[upload_media,number_of_speakers, input_lang, output_lang],
            outputs=[json_file, ranscript_box,]
        )
    ...
    with gr.Blocks() as demo:
        ...
        with gr.Row():
            with gr.Column():
                ...
                json_transcript = gr.Textbox(label="Paste the Json Transcription", lines=4)
            with gr.Column():
        ...
        generate_btn.click(
            fn=prompt_maker,
            inputs=[json_transcript, language, task],
...
# def main(debug=True, share=True): 
  ...
  demo = gr.TabbedInterface([demo1, demo2], ["Generate Transcription", "Transcription Translate"], title="",theme=gr.themes.Soft(),css=custom_css)
